...
cfgOFDM = wlanGetOFDMConfig(cbw,'Long','HT',numSTS); % Get OFDM configuration
...
est = htltfEstimate(rxSym,cbw,numSTS,numESS,ind);
...
if enableFreqSmoothing
    ...
    est(:,1:numSTS,:) = wlanCyclicShift(est(:,1:numSTS,:), ...
                                        csh,FFTLen,'Rx',ind);
    cshEss = getCyclicShiftVal('VHT',numESS,chanBWInMHz);
    if numESS>1
        est(:,numSTS+(1:numESS),:) = ...
            wlanCyclicShift(est(:,numSTS+(1:numESS),:),cshEss, ...
                            FFTLen,'Rx',ind);
    end
    ...
    est(:,1:numSTS,:) = wlanCyclicShift(est(:,1:numSTS,:), ...
                                        csh,FFTLen,'Tx',ind);
    if numESS>1
        est(:,numSTS+(1:numESS),:) = wlanCyclicShift(est(:, ...
